...
NOMES_CONHECIDOS = ["Bruna", "Madê", "Luana", "Diego", "Francely", "Jarlene", "Lizia", "Pierri", "Elzenir", "Matheus", "Felipe", "Márcia", "André", "Outro"]

DEFAULTS_PLACAR = {
    "pmw": {"ml": 610000, "mf": 120000, "vl": 0, "vf": 0, "dias": 0},
    "slz": {"ml": 415000, "mf": 85000, "vl": 0, "vf": 0, "dias": 0},
    "itz": {"ml": 335000, "mf": 45000, "vl": 0, "vf": 0, "dias": 0},
}
...
def carregar_tudo():
    ...
    # compatibilidade: bin antigo guardava o placar direto na raiz (sem "placar")
    if "placar" in bruto:
    ...
    with st.form("form_placar"):
        ...
        for col, store in zip(cols, STORES):
            with col:
                ...
                sid = store["id"]
                atual = dados.get(sid, DEFAULTS_PLACAR[sid])
                ...
                dias_loja = st.number_input("Dias úteis passados", min_value=0, max_value=31, value=int(atual.get("dias", 0)), key=f"dias_{sid}")
                novo_placar[sid] = {"ml": ml, "mf": mf, "vl": vl, "vf": vf, "dias": dias_loja}
...
for store in STORES:
    sid = store["id"]
    valores = dados.get(sid, DEFAULTS_PLACAR[sid])
    ...
    init_lines.append(f'  document.getElementById("dias_{sid}").value = {int(valores.get("dias", 0))};')
...
with st.form("form_comentario", clear_on_submit=True):
    ...
    with c2:
        texto = st.text_input("Comentário", placeholder="Ex: Palmas tá na frente hahah 🔥")
    ...
    if enviar_comentario and texto.strip():
        ...
        comentarios = ([novo_comentario] + comentarios)[:50]  # guarda só os 50 mais recentes
